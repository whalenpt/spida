...
[![Build](https://github.com/whalenpt/spida/actions/workflows/cmake.yml/badge.svg)](https://github.com/whalenpt/spida/actions/workflows/cmake.yml)
[![Coverage](https://codecov.io/gh/whalenpt/spida/branch/main/graph/badge.svg?token=79G4BWT348)](https://codecov.io/gh/whalenpt/spida)
...
2. Fast Discrete Cosine Transforms (FDCTs)
3. Hankel-Transforms
...
Non-diagonal methods are not implemented as of yet (see [rkstiff](https://github.com/whalenpt/rkstiff) for this functionality).
...
If not installed, first install [CMake](https://cmake.org/).
...
[Conan](https://conan.io) is the recommended way to manage third-party dependencies
...
- Build artifacts written to `/workspace/build/` appear on the host at `spida/build/` — useful for inspecting binaries or compile commands.
- To keep a container alive across multiple sessions, drop `--rm` and use `docker start -ai <name>` to re-attach.
- Re-run `docker build -t spida .` whenever `Dockerfile` or `conanfile.py` changes to refresh the cached packages.
