...
{
    public:
        PropagatorKDV(const std::filesystem::path& path,kDV_RV& md) : spida::PropagatorCV(path),
            m_spi(md.spida()),
...
int main()
{
    ...
    spida::UniformGridRVX grid(nx,minx,maxx);
    kDV_RV model(grid);
    ...
    if(!solver.evolve(propagator,0.0,600.0,0.1)) { std::cerr << "Solver failed\n"; return 1; }
