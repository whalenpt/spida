...
void SolverCV::setLogFrequency(unsigned val)
{
    m_stat->setLogFrequency(val);
}

void SolverCV::setNumThreads(unsigned val)
{
    m_thmgt->setNumThreads(val);
}

unsigned SolverCV::numThreads() const
{
...
double Control::computeS(const std::vector<dcmplx>& errVec,
...
double Control::computeRawS(const std::vector<dcmplx>& errVec,
                            const std::vector<dcmplx>& ynew) const noexcept
{
    double enorm = 0.0;
    double ynorm = 0.0;
