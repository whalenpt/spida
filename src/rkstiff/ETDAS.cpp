...
ETD34::ETD34(const LinOp& Lop, const NLfunc& NL, bool use_refs)
    : SolverAS_ETD(Lop, NL, 0.84, 4, use_refs),
      m_sz(SolverCV::size()),
      L(SolverCV::L()),
      EL(m_sz),
      EL2(m_sz),
...
void ETD34::worker_coeff(double dt, int tid)
{
    ...
    for (unsigned i = tid; i < m_sz; i += SolverCV::threadManager().getNumThreads()) {
        ...
        if (abs(Lval) < modeCutoff()) {
            ...
            dcmplx a31val(0.0, 0.0);
            dcmplx a32val(0.0, 0.0);
            dcmplx a41val(0.0, 0.0);
            dcmplx a43val(0.0, 0.0);
            dcmplx a51val(0.0, 0.0);
            dcmplx a52val(0.0, 0.0);
            dcmplx a54val(0.0, 0.0);
            for (unsigned j = 0; j < contourPoints(); j++) {
                dcmplx z(Lval + r[j]);
            ...
            a51[i] =
                dt * (-4.0 - Lval + exp(Lval) * (4.0 - 3.0 * Lval + pow(Lval, 2))) / pow(Lval, 3);
            a52[i] = 2.0 * dt * (2.0 + Lval + exp(Lval) * (-2.0 + Lval)) / pow(Lval, 3);
            a54[i] =
                dt * (-4.0 - 3.0 * Lval - pow(Lval, 2) + exp(Lval) * (4.0 - Lval)) / pow(Lval, 3);
        }
    }
}
...
void ETD34::updateStages(const std::vector<dcmplx>& in,
...
void ETD35::worker_coeff(double ds, int tid)
{
    ...
    for (unsigned i = tid; i < m_sz; i += SolverCV::threadManager().getNumThreads()) {
        ...
        if (abs(Lval) < modeCutoff()) {
            ...
            dcmplx a52val(0.0, 0.0);
            dcmplx a54val(0.0, 0.0);
            dcmplx a61val(0.0, 0.0);
            dcmplx a62val(0.0, 0.0);
            dcmplx a63val(0.0, 0.0);
            dcmplx a65val(0.0, 0.0);
            dcmplx a71val(0.0, 0.0);
            dcmplx a73val(0.0, 0.0);
            dcmplx a74val(0.0, 0.0);
            dcmplx a75val(0.0, 0.0);
            dcmplx a76val(0.0, 0.0);
            for (unsigned j = 0; j < contourPoints(); j++) {
                dcmplx z(Lval + r[j]);
                ...
                a63val +=
                    (3.0 * (58.0 + 21.0 * z + exp(z) * (-58.0 + 37.0 * z))) / (28.0 * pow(z, 2));
                a65val += (-286.0 - 239.0 * z + exp(z) * (286.0 - 47.0 * z)) / (84.0 * pow(z, 2));
                a71val += (7.0 * (-1620.0 - 626.0 * z - 73.0 * pow(z, 2) +
                                  exp(z) * (1620.0 - 994.0 * z + 257.0 * pow(z, 2)))) /
                          (2700.0 * pow(z, 3));
                a73val += (900.0 + 1834.0 * z + 287.0 * pow(z, 2) +
                           exp(z) * (-900.0 - 934.0 * z + 1097.0 * pow(z, 2))) /
                          (1350.0 * pow(z, 3));
                a74val += (2.0 * (810.0 + 412.0 * z + 56.0 * pow(z, 2) +
                                  exp(z) * (-810.0 + 398.0 * z - 49.0 * pow(z, 2)))) /
                          (225.0 * pow(z, 3));
                a75val += (540.0 - 1226.0 * z - 1183.0 * pow(z, 2) +
                           exp(z) * (-540.0 + 1766.0 * z - 313.0 * pow(z, 2))) /
                          (1350.0 * pow(z, 3));
                a76val += (-10980.0 - 6722.0 * z - 1741.0 * pow(z, 2) +
                           exp(z) * (10980.0 - 4258.0 * z + 509.0 * pow(z, 2))) /
                          (2700.0 * pow(z, 3));
            }
            a21[i] = mult * a21val;
            a31[i] = mult * a31val;
            ...
            a51[i] = ds * (4.0 + Lval + 2.0 * exp((3.0 * Lval) / 4.0) * (-2.0 + Lval)) /
                     (2.0 * pow(Lval, 2));
            a52[i] = ds * -(-1.0 + exp((3.0 * Lval) / 4.0)) / (2.0 * Lval);
            a54[i] =
                ds * -(4.0 - 4.0 * exp((3.0 * Lval) / 4.0) + 3.0 * Lval) / (2.0 * pow(Lval, 2));
            a61[i] = ds * (-118.0 - 41.0 * Lval + exp(Lval) * (118.0 - 77.0 * Lval)) /
                     (42.0 * pow(Lval, 2));
            a62[i] = ds * (8.0 * (-1.0 + exp(Lval))) / (7.0 * Lval);
            a63[i] = ds * (3.0 * (58.0 + 21.0 * Lval + exp(Lval) * (-58.0 + 37.0 * Lval))) /
                     (28.0 * pow(Lval, 2));
            a65[i] = ds * (-286.0 - 239.0 * Lval + exp(Lval) * (286.0 - 47.0 * Lval)) /
                     (84.0 * pow(Lval, 2));
            a71[i] = ds *
                     (7.0 * (-1620.0 - 626.0 * Lval - 73.0 * pow(Lval, 2) +
                             exp(Lval) * (1620.0 - 994.0 * Lval + 257.0 * pow(Lval, 2)))) /
                     (2700.0 * pow(Lval, 3));
            a73[i] = ds *
                     (900.0 + 1834.0 * Lval + 287.0 * pow(Lval, 2) +
                      exp(Lval) * (-900.0 - 934.0 * Lval + 1097.0 * pow(Lval, 2))) /
                     (1350.0 * pow(Lval, 3));
            a74[i] = ds *
                     (2.0 * (810.0 + 412.0 * Lval + 56.0 * pow(Lval, 2) +
                             exp(Lval) * (-810.0 + 398.0 * Lval - 49.0 * pow(Lval, 2)))) /
                     (225.0 * pow(Lval, 3));
            a75[i] = ds *
                     (540.0 - 1226.0 * Lval - 1183.0 * pow(Lval, 2) +
                      exp(Lval) * (-540.0 + 1766.0 * Lval - 313.0 * pow(Lval, 2))) /
                     (1350.0 * pow(Lval, 3));
            a76[i] = ds *
                     (-10980.0 - 6722.0 * Lval - 1741.0 * pow(Lval, 2) +
                      exp(Lval) * (10980.0 - 4258.0 * Lval + 509.0 * pow(Lval, 2))) /
                     (2700.0 * pow(Lval, 3));
        }
    }
}
...
void ETD35::worker_stage7(const std::vector<dcmplx>& in,
                          std::vector<dcmplx>& ynew,
