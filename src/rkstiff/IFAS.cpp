...
void IF34::updateStages(const std::vector<dcmplx>& in,
...
void IF45DP::updateStages(const std::vector<dcmplx>& in,
                          std::vector<dcmplx>& ynew,
                          std::vector<dcmplx>& errVec) noexcept
{
    if (!N1_init) {
    ...
    for (unsigned i = 0; i < m_sz; i++)
        tempK[i] =
            EL5[i] * in[i] + a51[i] * N1[i] + a52[i] * N2[i] + a53[i] * N3[i] + a54[i] * N4[i];
    ...
    for (unsigned i = 0; i < m_sz; i++)
        errVec[i] =
            r1[i] * N1[i] + r3[i] * N3[i] + r4[i] * N4[i] + r5[i] * N5[i] + r6 * N6[i] + r7 * N7[i];
