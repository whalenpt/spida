...
const UniformGridRVX& SpidaRVX::getGridX() const
{
    return *m_gr;
}

FFTRVX& SpidaRVX::getTransformX()
{
    return *m_tr;
}

const std::vector<double>& SpidaRVX::getX() const
{
    return m_gr->getX();
}

const std::vector<double>& SpidaRVX::getSX() const
{
    return m_gr->getSX();
}

void SpidaRVX::X_To_SX(const std::vector<double>& in, std::vector<dcmplx>& out) noexcept
{
    m_tr->X_To_SX(in, out);
}

void SpidaRVX::SX_To_X(const std::vector<dcmplx>& in, std::vector<double>& out) noexcept
{
    m_tr->SX_To_X(in, out);
}
