...
using chronoMap =
    std::map<std::string, std::chrono::time_point<std::chrono::system_clock>, std::less<>>;
...
public:
    ...
    void addTimer(std::string const& str)
    {
        m_timer.addTimer(str);
    }

    void startTimer(const std::string& str)
    {
        m_timer.startTimer(str);
    }

    bool endTimer(const std::string& str)
    {
        return m_timer.endTimer(str);
    }

    void addClock(const std::string& str)
    {
        m_clocker.addClock(str);
    }

    void startClock(const std::string& str)
    {
        m_clocker.startClock(str);
    }

    bool endClock(const std::string& str)
    {
        return m_clocker.endClock(str);
    }

    void addTracker(const std::string& str, double val = 0.0)
    {
        m_tracker.addTracker(str, val);
    }

    void updateTracker(const std::string& str, double val)
    {
        m_tracker.updateTracker(str, val);
    }

    void setLogFrequency(unsigned val);

    void setHeader(std::string_view str)
    {
        m_name = str;
    }
