...
inline void createDirectory(const std::filesystem::path& dir_path, bool overwrite = true) {
    if (overwrite) clearDirectory(dir_path);
    if (!std::filesystem::exists(dir_path))
...
public:
    ...
    void setItem(const std::string& key, double val) {
        std::ostringstream oss; oss << std::setprecision(12) << val;
        m_metadata_map[key] = oss.str();
    }
    void setItem(const std::string& key, float val) {
        std::ostringstream oss; oss << std::setprecision(6) << val;
        m_metadata_map[key] = oss.str();
    }
    ...
    void setItem(const std::string& key, const std::string& nm) { m_metadata_map[key] = nm; }
    void removeItem(const std::string& key) { m_metadata_map.erase(key); }
    ...
    [[nodiscard]] const metadataMap& getMetadata() const { return m_metadata_map; }
    [[nodiscard]] std::filesystem::path dirpath() const { return m_dirpath; }
...
public:
    ...
    [[nodiscard]] unsigned getStrideX() const { return m_strideX; }
    [[nodiscard]] unsigned getStrideY() const { return m_strideY; }
...
public:
    ...
    [[nodiscard]] TrackType getTrackType() const { return m_ttype; }
...
inline nlohmann::json buildMeta(const metadataMap& meta) {
    ...
    for (auto const& [k, v] : meta) {
        try {
            ...
            if (pos == v.size()) j[k] = d; else j[k] = v;
        } catch (...) { j[k] = v; }
    }
    return j;
}
...
public:
    ...
    ReportComplex1D(const std::string&, std::vector<Tx>&&,
                    std::vector<std::complex<Ty>>&&) = delete;
    ReportComplex1D(const std::string&, std::vector<Tx>&&,
                    const std::vector<std::complex<Ty>>&) = delete;
    ReportComplex1D(const std::string&, const std::vector<Tx>&,
                    std::vector<std::complex<Ty>>&&) = delete;

    void setPower(bool v) { m_power = v; }
    [[nodiscard]] bool getPower() const { return m_power; }
private:
    const std::vector<Tx>& m_x;
    ...
    void reportImplement(std::ofstream& os) const override {
        if (m_power) reportPower(os); else reportComplex(os);
    }
};
...
public:
    ...
    Report2D(const std::string&, std::vector<Tx>&&,
             std::vector<Ty>&&, std::vector<Tz>&&) = delete;
private:
    const std::vector<Tx>& m_x;
    const std::vector<Ty>& m_y;
...
template<typename Tx, typename Ty, typename Tz>
Report2D(const std::string&, const std::vector<Tx>&,
         const std::vector<Ty>&, const std::vector<Tz>&) -> Report2D<Tx, Ty, Tz>;
...
public:
    ...
    ReportComplex2D(const std::string&, std::vector<Tx>&&, std::vector<Ty>&&,
...
private:
    ...
    void buildAxes(std::size_t sx, std::size_t sy,
                   std::vector<Tx>& xs, std::vector<Ty>& ys) const {
        xs.reserve(pw::intceil(m_x.size(), sx));
        ys.reserve(pw::intceil(m_y.size(), sy));
        for (std::size_t i = 0; i < m_x.size(); i += sx) xs.push_back(m_x[i]);
        for (std::size_t j = 0; j < m_y.size(); j += sy) ys.push_back(m_y[j]);
