...
// range."
template <typename T> unsigned nearestIndex(const std::vector<T>& sorted_vec, T val)
{
    if (val < sorted_vec[0]) {
        std::string msg = "nearestIndex(T val,vector<T>& sorted_vec) error: the val of " +
                          std::to_string(val) + " is less than the minimum xval of " +
                          std::to_string(sorted_vec[0]) + ".";
        throw std::out_of_range(msg);
    }
    else if (val > sorted_vec.back()) {
        std::string msg = "nearestIndex(T val,vector<T>& sorted_vec) error: the val of " +
                          std::to_string(val) + " is greater than the maximum xval of " +
                          std::to_string(sorted_vec.back()) + ".";
        throw std::out_of_range(msg);
    }
