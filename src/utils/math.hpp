...
{
    T squared_sum = std::accumulate(
        std::begin(v), std::end(v), 0.0, [](auto sum, auto& val) { return sum + std::norm(val); });
    return sqrt(squared_sum);
}

template <typename T> auto max_element(const std::vector<std::complex<T>>& v)
{
...
template <typename T> auto min_element(const std::vector<std::complex<T>>& v)
{
    auto dcmplx_lt = [](auto a, auto b) { return abs(a) < abs(b); };
...
template <typename T> auto norm(const std::vector<T>& v)
{
    T squared_sum = std::accumulate(
        std::begin(v), std::end(v), 0.0, [](auto sum, auto& val) { return sum + pow(val, 2); });
    return sqrt(squared_sum);
}

template <typename T> auto max(const std::vector<T>& v)
{
    return *std::max_element(std::begin(v), std::end(v));
}

template <typename T> auto argmax(const std::vector<T>& v)
{
    auto itmax = std::max_element(std::begin(v), std::end(v));
    return std::distance(std::begin(v), itmax);
}

template <typename T> auto min(const std::vector<T>& v)
{
    return *std::min_element(std::begin(v), std::end(v));
}

template <typename T> auto argmin(const std::vector<T>& v)
{
