...
void checkXInterp(const std::vector<double>& xinterp,const std::vector<double>& xdata)
{
    ...
    if(xinterp[0] < xdata[0])
        throw pw::Exception(std::format(
            ...
            "m_xvec[0], increase xinterp[0] to at least {}", xdata[0]));
...
void checkXInterp(double xinterp,const std::vector<double>& xdata)
{
    if(xinterp < xdata[0])
        throw pw::Exception(std::format(
            "eval(double xinterp) failed: xinterp is less than the class m_xvec[0], "
            "increase xinterp[0] to at least {}", xdata[0]));
    if(xinterp > xdata.back())
        throw pw::Exception(std::format(
            "eval(double xinterp) failed: xinterp is greater than the class m_xvec[m_size-1], "
            "decrease xinterp to below or equal to {}", xdata.back()));
}
        ...
        const std::vector<double>& c,const std::vector<double>& d,std::vector<double>& x)
{
    ...
    if(a.size() != b.size()-1)
        throw pw::Exception(std::format(
            "tridisolve(a,b,c,d,x) error: subdiagonal 'a' size of {} must be equal to "
            "the size of the diagonal 'b' {} minus one", a.size(), b.size()));
    if(c.size() != b.size()-1)
        throw pw::Exception(std::format(
            "tridisolve(a,b,c,d,x) error: superdiagonal 'c' size of {} must be equal to "
            "the size of the diagonal 'b' {} minus one", c.size(), b.size()));
    if(b.size() != d.size())
        throw pw::Exception("tridisolve(a,b,c,d,x) error: diagonal 'b' size must equal R.H.S. 'd' size");
    auto sz = b.size();
    x.assign(sz,0.0);
    ...
    m_dk(xvec.size()),m_ck(xvec.size()-1),m_bk(xvec.size()-1)
{
    checkData(xvec,yvec);
    if(xvec.size() < 3)
        throw pw::Exception("SplineInterp constructor error: xvec size must be at least 3 for cubic spline");
    this->initializeCoefficients();
}

void SplineInterp::buildTridiSystem(const std::vector<double>& h,const std::vector<double>& delta,
...
double SplineInterp::eval(double xinterp) const
{
    ...
    return this->m_yvec[idx-1] + s*this->m_dk[idx-1]
            + s*s*this->m_ck[idx-1] + s*s*s*this->m_bk[idx-1];
}
