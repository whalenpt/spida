...
PropagatorsRTp_RC::PropagatorsRTp_RC(spida::SpidaRT* ida,const double* plas,pw::ParamBin& bin) 
  : Propagators(ida,bin), spi(ida), pl(plas)
{
  ...
  double vg = bin.getDbl("vg");
  double n0 = bin.getDbl("n0");
  vars.add("n0",n0);
  vars.add("k0",k0);
  ...
  using uppe::EPS0;
  bin.add("INPUT/E0",sqrt(2.0*bin.getDbl("INPUT/Intensity")/(LIGHT_SPEED*EPS0*n0)));
  InitPulse(bin);
  InitReport(bin);
}

PropagatorsRTp_RC::~PropagatorsRTp_RC()
{
  delete [] U;
  delete [] V;
  delete [] I;
...
void PropagatorsRTp_RC::InitPulse(pw::ParamBin& bin)
{
...
void PropagatorsRTp_RC::updateFields(double z)
{
  using uppe::dcmplx;
  using uppe::ii;
  using uppe::LIGHT_SPEED;
...
void PropagatorsRTp_RC::InitReport(pw::ParamBin& bin)
{
  int nt = spi->getNT();
  int nr = spi->getNr();
  int nkr = spi->getNsr();
  ...
  if(bin.getBool("REPORT/PeakPlasma")){
    ReportMax_R* def = new ReportMax_R("PeakPlasma",pl,nr,nt);
    fields::setPeakPlasma(def,bin,scales);
    rp->addReport(def);
  }

  if(bin.getBool("REPORT/OnAxisPlasma")){
    ReportMax_R* def = new ReportMax_R("OnAxisPlasma",pl,nr,nt);
    def->setOnAxisD2(0);
    fields::setOnAxisPlasma(def,bin,scales);
    rp->addReport(def);
  }

  if(bin.getBool("REPORT/RhoT")){
    ReportDef1D_R* def = new ReportDef1D_R("RhoT",spi->getT(),pl,nr,nt);
    def->setOnAxisD2(0);
    fields::setRhoT(def,bin,scales);
    rp->addReport(def);
  }

  if(bin.getBool("REPORT/RhoRT")){
    ReportDef2D_R* def = new ReportDef2D_R("RhoRT",spi->getR(),spi->getT(),pl,nr,nt);
    fields::setRhoRT(def,bin,scales);
    rp->addReport(def);
  }
