...
void FFTRVT::T_To_ST(const double* in, dcmplx* out)
{
    ...
    // Divide by FFT multiplier m_nt and adjust phase since physical grid is not assumed to start at
    // 0 Also take negative imaginary component -> (-iwt convention vs kiss fft convention of iwt)
    for (auto j = m_minI; j <= m_maxI; j++) {
        dcmplx tempval((m_L * exp(ii * m_omega[j - m_minI] * m_mint)) * m_temp[j] /
                       static_cast<double>(m_nt));
...
{
    kiss_fft(m_cfg_forward,
             reinterpret_cast<const kiss_fft_cpx*>(in),
             reinterpret_cast<kiss_fft_cpx*>(m_cFFT.data()));
    for (unsigned j = m_minI; j <= m_maxI; j++)
        out[j - m_minI] =
            m_L * exp(ii * m_omega[j - m_minI] * m_mint) * m_cFFT[j] / static_cast<double>(m_nt);
}
