...
HankelTransformR::HankelTransformR(const BesselRootGridR& grid)
    : m_nr(grid.getNr()), m_Ymk(m_nr * m_nr), m_YmkC(m_nr * m_nr)
{
...
#if defined(HAVE_OPENBLAS)
    cblas_dgemv(
        CblasRowMajor, CblasNoTrans, m_nr, m_nr, m_alpha, m_Ymk.data(), m_nr, in, 1, 0.0, out, 1);
#else
...
void HankelTransformR::SR_To_R(const dcmplx* in, dcmplx* out) const
{
    ...
    cblas_zgemv(
        CblasRowMajor, CblasNoTrans, m_nr, m_nr, &a, m_YmkC.data(), m_nr, in, 1, &beta, out, 1);
#else
    for (unsigned k = 0; k < m_nr; k++) {
        dcmplx sum = 0.0;
        for (unsigned m = 0; m < m_nr; m++)
...
HankelTransformRb::HankelTransformRb(const BesselRootGridR& grid, unsigned threads)
    : m_threads(threads), m_nr(grid.getNr()), m_Ymk(grid.getNr() * grid.getNr())
{
    m_alpha = grid.getjN() / pow(grid.getMaxSR(), 2);
    initDHT(grid);
}
