...
void HankelFFTRCVT::RT_To_RST(const std::vector<dcmplx>& in, std::vector<dcmplx>& out)
{
    ...
    for (unsigned tid = 0; tid < m_threads; tid++) {
        workers.push_back(
            std::thread(&HankelFFTRCVT::worker_T_To_ST, this, tid, in.data(), out.data()));
    }
...
void HankelFFTRCVT::RST_To_RT(const std::vector<dcmplx>& in, std::vector<dcmplx>& out)
{
    ...
    for (unsigned tid = 0; tid < m_threads; tid++) {
        workers.push_back(
            std::thread(&HankelFFTRCVT::worker_ST_To_T, this, tid, in.data(), out.data()));
    }
    wait_for_workers(workers);
}
...
void HankelFFTRCVT::hSTR_To_STSR(const std::vector<dcmplx>& in, std::vector<dcmplx>& out)
{
    ...
    for (unsigned tid = 0; tid < m_threads; tid++) {
        workers.push_back(
            std::thread(&HankelFFTRCVT::worker_R_To_SR, this, tid, in.data(), out.data()));
    }
    wait_for_workers(workers);
}
...
void HankelFFTRCVT::hSTSR_To_STR(const std::vector<dcmplx>& in, std::vector<dcmplx>& out)
{
    ...
    for (unsigned tid = 0; tid < m_threads; tid++) {
        workers.push_back(
            std::thread(&HankelFFTRCVT::worker_SR_To_R, this, tid, in.data(), out.data()));
    }
    wait_for_workers(workers);
}
...
void HankelFFTRCVT::RT_To_SRT(const std::vector<dcmplx>& in, std::vector<dcmplx>& out)
{
    ...
    for (unsigned tid = 0; tid < m_threads; tid++) {
        workers.push_back(
            std::thread(&HankelFFTRCVT::worker_R_To_SR, this, tid, m_rr.data(), m_sr.data()));
    }
    wait_for_workers(workers);
...
void HankelFFTRCVT::SRT_To_RT(const std::vector<dcmplx>& in, std::vector<dcmplx>& out)
{
    ...
    for (unsigned tid = 0; tid < m_threads; tid++) {
        workers.push_back(
            std::thread(&HankelFFTRCVT::worker_SR_To_R, this, tid, m_sr.data(), m_rr.data()));
    }
    wait_for_workers(workers);
