...
TEST(KISS_TEST, RANDOM)
{
    ...
    double in_norm = std::accumulate(
        in.cbegin(), in.cend(), 0.0, [](double const& sum, const std::complex<double>& val) {
    ...
    double after_norm = std::accumulate(
        in.cbegin(), in.cend(), 0.0, [](double const& sum, const std::complex<double>& val) {
            return sum + std::norm(val);
        });
...
TEST(KISS_TEST, RANDOM_REAL)
{
    ...
    kiss_fftr(rcfg_forward,
              reinterpret_cast<kiss_fft_scalar*>(in.data()),
              reinterpret_cast<kiss_fft_cpx*>(out.data()));
    kiss_fftri(rcfg_reverse,
               reinterpret_cast<kiss_fft_cpx*>(out.data()),
               reinterpret_cast<kiss_fft_scalar*>(in.data()));
    ...
    auto after_norm =
        std::accumulate(in.cbegin(), in.cend(), 0.0, [](double const& sum, const double& val) {
...
TEST(KISS_TEST, ZEROS)
{
    ...
    auto zero_sum = std::accumulate(
        out.cbegin(), out.cend(), 0.0, [](double const& sum, const std::complex<double>& val) {
            return sum + std::norm(val);
        });
